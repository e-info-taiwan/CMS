...
**最新指示（2026-09-23）：CLIP 匯入持續執行，pHash 已依使用者新指示恢復 8 個 task，
跳過已寫入的 24,334 筆 pHash。今年的圖片標籤 apply 尚未啟動，維持暫停，
不得依下方範例自行恢復。prod 圖片 AI 欄位與報題建議先隱藏。**

部署狀態（2026-09-23）：正式 DB 五個 migrations 已成功完成，Tag 的
4,353 筆有效名稱向量已補齊（另 1 筆空白名稱不處理）。圖片已通過 50 筆試跑。
本次回填改為沿用 lab CLIP 向量、全圖庫 pHash、2026 年圖片標籤。
映像 digest、build ID 與執行紀錄記錄在 `preparation.json`；Job 建立不代表已執行完成。
...
| `eic-photo-ai-backfill-prod` | 今年缺少的向量／Vision 建議標籤，不計算 pHash | 1 task |

Project：`mimetic-sweep-456508-k4`；region：`asia-east1`。
向量匯入完成後才執行今年 AI 回填，已存在的向量不重算。
